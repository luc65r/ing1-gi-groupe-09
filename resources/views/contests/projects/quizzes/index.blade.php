                                ...
                                <td class="flex justify-center text-center">
                                    @is('manager')
                                        <form action="{{ route('quizzes.destroy', $quiz->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button class="voirP mr-2 rounded-lg" type="submit" class="btn btn-danger"
                                                onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce quiz ?')">Supprimer</button>
                                        </form>
                                        <a class="voirP voirP mr-2 rounded-lg" href="{{ route('quizzes.edit', ['project' => $project, 'quiz' => $quiz->id]) }}">Modifier
                                        </a>
                                        <a class="voirP rounded-lg" href="{{ route('quizzes.responses', ['quiz' => $quiz]) }}">Voir les réponses</a>
                                    @else
                                        <a class="voirP rounded-lg" href="{{ route('quizzes.show', ['quiz' => $quiz]) }}">
                                            {{'Voir les questions' }}
                                        </a>
                                    @endis
